//
//  DetailedArticleView.swift
//  My AIM App Swiftui
//
//  Created by Emmanuel Donkor on 25/03/2023.
//

import SwiftUI

struct DetailedArticleView: View {
    let article: Article
    @State var articleDate: Date = .now
    
    var body: some View {
        ScrollView {
            AsyncImage(url: article.urlToImage, content: { phase in
                switch phase {
                case .empty :
                    ProgressView()
                        .frame(height: 300)
                        .padding(.vertical)
                case .success(let image):
                    image
                        .resizable()
                        .frame(height: 300)
                        .aspectRatio(contentMode: .fit)
                        .cornerRadius(20)
                        .padding(.bottom)
                case .failure:
                    Image(systemName: "exclamationmark.triangle.fill")
                        .resizable()
                        .padding()
                        .frame(width: 300, height: 300)
                        .background(RoundedRectangle(cornerRadius: 10)
                            .foregroundColor(.gray))
                        .cornerRadius(10)
                        .padding(.horizontal, 10)
                default:
                    Image(systemName: "exclamationmark.triangle.fill")
                        .resizable()
                        .padding()
                        .frame(width: 300, height: 300)
                        .background(RoundedRectangle(cornerRadius: 10)
                            .foregroundColor(.gray))
                        .cornerRadius(10)
                        .padding(.horizontal, 10)
                }
            })
            .frame(height: 300)
            .padding(.vertical)
            
            
            VStack(alignment: .leading) {
                Text(article.title)
                    .font(.title)
                    .fontWeight(.semibold)
                Text("By: " + (article.author ?? "Unknown Author"))
                    .font(.headline)
                Text(articleDate.formatted(date: .abbreviated, time: .standard))
                    .font(.caption)
                    .fontDesign(.monospaced)
                Divider()
                Text(article.content ?? "Empty... :(")
            }
            
            HStack {
                Text("Source: " + (article.source.name ?? "Unknown Source"))
                    .font(.footnote)
                    .foregroundColor(.black.opacity(0.7))
                    .padding()
                    .background(Rectangle()
                        .frame(width: UIScreen.main.bounds.width)
                        .foregroundColor(.gray))
                    .cornerRadius(30)
            }
        }
        .padding()
        .onAppear {
            articleDate = getArticleDate()
        }
    }
    
    func getArticleDate() -> Date {
        let dateformatter = DateFormatter()
        dateformatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ssZ"
        var articleDate: Date = Date()
        
        if let date = dateformatter.date(from: article.publishedAt) {
            articleDate = date
            print("Successfull")
        } else {
            print("There's a problem with the date string")
        }
        
        return articleDate
    }
}

struct DetailedArticleView_Previews: PreviewProvider {
    static var previews: some View {
        DetailedArticleView(article: Article(source: Source(id: nil, name: "ME hahahahah"), author: "Author", title: "Title", description: nil, urlToImage: URL(string: "data:image/png;base64,"), publishedAt: "2023-03-26T12:01:00Z", content: "Content goes here....!!!"))
    }
}
