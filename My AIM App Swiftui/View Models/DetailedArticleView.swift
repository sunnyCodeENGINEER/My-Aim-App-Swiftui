//
//  DetailedArticleView.swift
//  My AIM App Swiftui
//
//  Created by Emmanuel Donkor on 25/03/2023.
//

import SwiftUI

struct DetailedArticleView: View {
    @State var title: String = "Title"
    @State var author: String = "Author"
    @State var dateString: String = "2023-03-26T12:01:00Z"
    @State var content: String = "Content goes here."
    let urlToImage: URL?
    @State var articleDate: Date = .now
    
    var body: some View {
        ScrollView {
            AsyncImage(url: urlToImage, content: { phase in
                switch phase {
                case .empty :
                    ProgressView()
                        .frame(height: 300)
                        .padding(.vertical)
                case .success(let image):
                    image
                        .resizable()
                        .frame(height: 300)
                        .aspectRatio(contentMode: .fit)
                        .cornerRadius(20)
                        .padding(.vertical)
                case .failure:
                    Image(systemName: "exclamationmark.triangle.fill")
                        .resizable()
                        .padding()
                        .frame(width: 300, height: 300)
                        .background(RoundedRectangle(cornerRadius: 10)
                            .foregroundColor(.gray))
                        .cornerRadius(10)
                        .padding(.horizontal, 10)
                default:
                    Image(systemName: "exclamationmark.triangle.fill")
                        .resizable()
                        .padding()
                        .frame(width: 300, height: 300)
                        .background(RoundedRectangle(cornerRadius: 10)
                            .foregroundColor(.gray))
                        .cornerRadius(10)
                        .padding(.horizontal, 10)
                }
            })
            .frame(height: 300)
            .padding(.vertical)
            
            
            VStack(alignment: .leading) {
                Text(title)
                    .font(.title)
                    .fontWeight(.semibold)
                Text(author)
                    .font(.headline)
                Text(articleDate.formatted(date: .abbreviated, time: .standard))
                    .font(.caption)
                    .fontDesign(.monospaced)
                Divider()
                Text(content)
            }
        }
        .padding()
        .onAppear {
            articleDate = getArticleDate()
        }
    }
    
    func getArticleDate() -> Date {
        let dateformatter = DateFormatter()
        dateformatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ssZ"
        var articleDate: Date = Date()
        
        if let date = dateformatter.date(from: dateString) {
            articleDate = date
            print("Successfull")
        } else {
            print("There's a problem with the date string")
        }
        
        return articleDate
    }
}

struct DetailedArticleView_Previews: PreviewProvider {
    static var previews: some View {
        DetailedArticleView(urlToImage: URL(string: "data:image/png;base64,"))
    }
}
